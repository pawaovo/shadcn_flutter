// Ignored stock-only diagnostic: SDK selection intents handle arrow keys.
// Observers do not mutate input; all key events come from a real user.
import 'dart:convert';
...
@JS('window.__readonlySdkShortcutProbe')
...
class _ReadOnlyProbeState extends State<_ReadOnlyProbe> {
  ...
  void _record(String phase, {JSObject? event}) {
    ...
    final report = <String, Object?>{
      ...
      'selection_handler': 'Official Flutter SDK ExtendSelectionByCharacterIntent; no custom Actions or DOM selection writes',
      'control_text_variant': 'Single line, 109 UTF-16 code units; same as the previous HTML/Flutter controls',
      'build_mode': kReleaseMode ? 'release' : 'debug',
      'layout': 'Same left-column width as the previous stock control; right column intentionally empty',
      'latest': current,
    ...
    child: ColoredBox(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: <Widget>[
            const Text(
              'Stock readonly: Flutter SDK arrow intents',
              style: TextStyle(fontSize: 27, fontWeight: FontWeight.bold),
            ),
            const Text(
              'Click the stock text, press Meta/Ctrl+A, then ArrowRight once. This stock-only variant maps plain Left/Right and Shift+Left/Right to official Flutter selection intents. Readonly remains true; there is no custom character calculation, DOM selection write, autofocus, or synthetic key input. Passive logs compare Flutter and DOM state.',
            ),
            ...
            Row(
              ...
              children: <Widget>[
                Expanded(
                  child: _panel(
                    'Stock readonly EditableText with SDK selection actions',
                    Shortcuts(
                      ...
                      child: AnimatedBuilder(
                        ...
                        builder: (context, _) => MergeSemantics(
                          child: Semantics(
                            label: 'Stock SDK read-only document',
                            textField: true,
                            enabled: true,
                            readOnly: true,
                            focusable: true,
